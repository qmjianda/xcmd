...
static xnr_var_t g_var_buf[XCMD_VAR_BUF_SZIE];
...
{
    if (NULL == xcmder || NULL == fmt)
        return -1;
    char ucstring[XCMD_PRINT_BUF_MAX_LENGTH] = {0};
    va_list arg;
    va_start(arg, fmt);
    int len = vsnprintf(ucstring, XCMD_PRINT_BUF_MAX_LENGTH, fmt, arg);
    va_end(arg);
    return xcmder->io.write(xcmder->io.out_fd, ucstring, len);
}

int xcmd_puts(xcmder_t *xcmder, const char *str)
{
    return xnr_io_puts(&xcmder->io, str);
}

int xcmd_putc(xcmder_t *xcmder, char c)
{
    return xnr_io_putc(&xcmder->io, c);
}

int xcmd_getc(xcmder_t *xcmder)
{
    return xnr_io_getc(&xcmder->io);
}
...
void xcmd_init(xcmder_t *xcmder, _io_write_t write, _io_read_t read)
{
    if (write && read && xcmder && 0 == xcmder->_initOK)
    {
...
#endif
        xnr_var_init(&xcmder->var_tab, g_var_buf, XCMD_VAR_BUF_SZIE);
        ...
        xnr_var_set(&xcmder->var_tab, "prompt", ANSI_COLOR_TXT(ANSI_YELLOW, "->"));
        char *var_val = xnr_var_value(&xcmder->var_tab, "prompt");
        xnr_line_set_prompt(&xcmder->line, var_val);
