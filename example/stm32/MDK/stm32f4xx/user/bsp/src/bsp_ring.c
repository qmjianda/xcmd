...
#define SEMM_CREAT()    xSemaphoreCreateMutex()
#define SEMM_TAKE()     xSemaphoreTake( ringMutex, portMAX_DELAY );
#define SEMM_GIVE()     xSemaphoreGive(ringMutex);
#else
#define SEMM_CREAT() 
#define SEMM_TAKE() 
#define SEMM_GIVE() 
...
RING_ID xrngInit(T_Ring *ring ,char *buffer, int bufLen)
{
    ...
    #if ENABLE_OS
    ringMutex = SEMM_CREAT();
    if(ringMutex == NULL)
    {
        while(1)
        {
            printf("ring sem error\r\n");
            vTaskDelay(1000);
        }
    }
    #endif
